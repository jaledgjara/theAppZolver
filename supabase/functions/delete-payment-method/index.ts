...
serve(async (req) => {
  ...
  try {
    ...
    const { card_id, provider_customer_id, provider_card_id } = body;

    if (!card_id || !provider_customer_id || !provider_card_id) {
      return new Response(
        JSON.stringify({ success: false, error: "Missing required fields: card_id, provider_customer_id, provider_card_id" }),
    ...
    if (fetchError || !card) {
      return new Response(
        JSON.stringify({ success: false, error: "Tarjeta no encontrada." }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" }, status: 404 }
      );
    }

    if (card.user_id !== verifiedUid) {
      console.error(`[delete-payment-method] Ownership mismatch: card.user_id=${card.user_id}, jwt=${verifiedUid}`);
      return new Response(
    ...
    console.log(`[delete-payment-method] Deleting card ${provider_card_id} from MP customer ${provider_customer_id}`);
    ...
    console.error("[delete-payment-method] Exception:", error.message);
    return new Response(
